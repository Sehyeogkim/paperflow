...
GPR 대리모델 위에서 Saltelli 샘플링으로 1차 지수($S_1$)와 전차수 지수($S_T$)를 산출하였다(인자별·인자군별; **Fig. 7**). 개별 민감도를 분석하 (A)를 살펴보면, 지배 인자는 VI1,2에서 모두 **재료 모듈러스($E_\text{vessel}$, $E_\text{FC}$)와 수축기압($P_\text{sys}$)**이다. 또한, (B)의 그룹 민감도를 살펴보면, **두 표현형(LAP·CP)과 두 지수(VI1·VI2) 모두에서 **재료물성군(Material)이 지배적**이었다. 그리고 혈류학적 파라메터가 그담으로 높은 민감도를 보이며, 형태학적 파라메터의 경우, 낮은 개별민감도이지만 St가 높은 것으로 확인된다.
