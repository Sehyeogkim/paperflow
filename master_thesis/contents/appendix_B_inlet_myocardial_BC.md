...
(Fig B1 — Appendix B의 두 경험적 입력: (a) 유입파형 $Q(t)$ (`inflow.dat`, 한 cycle), (b) 정규화 elastance $e(x)=E/E_\text{max}$ (`elastance.dat`, $x=t/t_\text{sys}$; $x=1$이 수축기 말). $P_\text{in}(t)$·$P_\text{myo}(t)$는 이 두 곡선과 B.1·B.2 식 + Table B1로부터 유도되므로, 재현에 필요한 비유도 입력은 이 둘뿐이다. 생성 스크립트 `Appendix_code/fig_B_inputs.py`.)
...
2. **심장 ODE**: 심방($_\text{LA}$)–심실($_\text{LV}$)–대동맥 lumped 회로를 다음 상태계로 둔다. 상태변수는 용적 $V_\text{LV},V_\text{LA}$와 판막 유량 $Q_\text{ao},Q_\text{mv}$이며, 정맥환류 $Q_\text{ven}=\text{CO}$(일정), 대동맥 afterload $P_\text{ao}(t)=P_\text{in}(t)$(B.1)이다.

   $$\frac{dV_\text{LV}}{dt}=Q_\text{mv}-Q_\text{ao},\qquad \frac{dV_\text{LA}}{dt}=Q_\text{ven}-Q_\text{mv},$$

   $$P_\text{LV}(t)=E(t)\,V_\text{LV}(t),\qquad P_\text{LA}(t)=E_a\,V_\text{LA}(t),$$

   $$L_v\frac{dQ_\text{ao}}{dt}=P_\text{LV}-P_\text{ao}-R_v\,Q_\text{ao}\quad(\text{대동맥판 open}),\qquad
   L_a\frac{dQ_\text{mv}}{dt}=P_\text{LA}-P_\text{LV}-R_a\,Q_\text{mv}\quad(\text{승모판 open}).$$

   판막은 다이오드로 처리한다: 대동맥판은 $P_\text{LV}\ge P_\text{ao}$일 때, 승모판은 $P_\text{LA}\ge P_\text{LV}$일 때만 열리고, 닫히면 해당 유량을 $0$으로 둔다($Q_\text{ao},Q_\text{mv}\ge0$). 용적은 trapezoidal, 유량은 forward Euler로 전진시키며 매 스텝 판막 상태를 갱신한다. $Q(t)$를 $n$ cycle 반복해 주기정상상태에 이른 뒤, 마지막 cycle의 심실압을 $P_\text{myo}(t)=P_\text{LV}(t)$로 추출한다.
3. **주요 상수**: lumped 심장 모델의 elastance·저항·관성·심박 상수는 Table B1에 정리한다. 정규화 elastance $e(x)$는 Fig B1(b) 참조.

(Table B1 — Lumped 심장 모델 상수)

| 구분 | 파라미터 | 기호 | 값 | 단위 |
|---|---|---|---|---|
| 심실 | 최대 elastance | $E_\text{max}$ | 2.0 | mmHg/mL |
| 심실 | 수축기 지속 | $t_\text{sys}$ | 0.4 | s |
| 심실 | 저항 | $R_v$ | 40.0 | CGS |
| 심실 | 관성 | $L_v$ | 0.01 | CGS |
| 심방 | elastance | $E_a$ | 250 | CGS |
| 심방 | 저항 | $R_a$ | 5.0 | CGS |
| 심방 | 관성 | $L_a$ | 0.1 | CGS |
| 전역 | 심박수 | HR | 60 | bpm |
| 전역 | 심박출량 | CO | 5 | L/min |
| 초기조건 | 심실/심방 용적 | $V_v(0)/V_a(0)$ | 0 / −60 | mL |

<!-- 〔TODO〕 ① 〔확인〕 Table B1 단위: 코드(P_myo.py)는 R/L/E_atrial을 CGS로 둠($E_a=250$ → ≈0.19 mmHg/mL).
        본문 표기 단위(CGS vs mmHg계) 확정 후 'CGS' 셀을 구체 단위(dyne·s·cm⁻⁵ 등)로 교체.
     ② (선택) B.1 적합 결과값 표(대표 case의 R_p, R_d, C, P_d) → 본문 Table 〔X〕와 연결.
     ③ 유입파형 Q(t) 출처/형상 1문장 + 인용.
     ④ elastance 정규화 곡선 e(x) 출처 인용([cite: stergiopulos1996elastance] 보완 가능).
     ⑤ 〔확인〕 Fig B1(a) Q(t)는 systole-우세(t/T≈0.22 peak) → 'coronary/이완기 우세' 표현 재고,
        y축 단위 [mL/s] 확정. elastance.dat 확보됨(data/input_parameter/). -->
