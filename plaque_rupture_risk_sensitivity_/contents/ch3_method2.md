...
*석회화 부피 $V_\text{calc}$은 입력 변수가 아니라 derived 검증량이다(근거·범위는 §3.1.2).*
...
**석회화 형상 생성(CP 전용).** 석회화는 단순 형상 primitive가 아니라 lipid core 내부에서 **seed 기반 region-growing 알고리즘**으로 생성하여 불규칙한 임상적 형태를 재현하였다(전 과정은 Appendix A). 데이터 생성 시 석회화의 양은 분율 $f_\text{calc}$로, 위치·형상은 추가 생성 파라미터 4개로 각각 독립 LHS 샘플하여 형태 다양성을 확보하였으나, 민감도 분석 입력으로는 임상적으로 해석 가능한 $f_\text{calc}$만 사용하였다(나머지 4개의 기여는 §3.3.2에서 무시할 수준으로 확인). 생성된 석회화에서 측정한 부피 $V_\text{calc}$은 입력이 아니라 lipid core 구속으로 독립 샘플이 불가능한 derived 양이며, 그 분포를 임상 보고 범위 [21]와 대조해 샘플링의 생리학적 타당성을 검증(validation)하는 데 사용한다(CP 전반 $V_\text{calc}\in[5.14\times10^{-4},\,2.81\times10^{-2}]$ cm³).
...
| # | Clinical fact | Model feature (per case) | Ref |
|---|---|---|---|
| 1 | Collagen-deficient fibrous cap | Low $E_\text{FC}$ | [40] |
| 2 | Soft plaque substrate | Low $E_\text{lipid}$, low $E_\text{vessel}$ | [10] |
| 3 | Large lipid arc angle | Large $\theta_\text{lipid}$ | [41] |
| 4 | Large plaque burden | High lipid-core volume | [15] |
| 5 | Large trans-lesional fractional flow reserve (FFR) gradient | High ΔFFR | [42] |
| 6 | Non-hard plaque (CP only) | Low $f_\text{calc}$ | [43] |
| 7 | Thin fibrous cap | Low fibrous-cap thickness | [13] |

이 rubric으로 **admissible로 판정된 지표만을** 이후 surrogate 학습·민감도 분석의 대상으로 삼는다. 후보별·기준별 통과/탈락과 최종 채택 지표는 **Results §3.1에서 제시한다.**
...
는 상호작용을 포함한 총기여를 정량화한다. Saltelli 준난수 표본추출로 1·전차 지수를 추정하였고(2차 지수는 계산하지 않음), 표현형당 기저표본 $N=1{,}024$로 LAP는 $N(d+2)=14{,}336$, CP는 $16{,}384$회 surrogate 평가를 수행하였다 [45].
