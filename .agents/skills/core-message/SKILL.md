---
name: core-message
description: Workflow step 1. Elicit and lock the paper's ONE core message (1 sentence + 1 paragraph) before any drafting. Use right after journal-info setup, or when the author's direction feels vague / the paper has drifted. Writes the result to <paper>/1_coremessage.md.
argument-hint: "[paper dir, e.g. master_thesis]"
---

# /core-message — Step 1: lock the core message
...
- If `<paper>/1_coremessage.md` already has content, read it and treat this as a *refine* session, not a blank start.
...
## 4. Save
Write the confirmed message to `<paper>/1_coremessage.md` in this shape:
